...
* @Last Modified time: 2017-04-13 18:20:08
*/
...
$(function(){
	...
	function creatDom(indexData,pfid){
		...
		for (var i=0,leng=anchorlists.length;i<leng;i++) {
			...
			if(isGot=="0"){
				...
				str_html+="<p>尚未收到"+anchorindex.name+"</p>";
				// console.log("没有收到礼物");
			// 如果收到礼物了
			}else{
	...
	//一、 动态DOM事件
	// 2、点击头像弹出贡献者信息
	$(document).on('click', '.open-contributors', function(event) {
	...
	//3、点击已经上传的照片铺满整个屏幕
	$(document).on('click', '.same-bg', function(event) {
		event.preventDefault();
		event.stopPropagation();
		event.preventDefault();
		event.stopPropagation();
	...
	//4 点击弹出上传照片的弹出框
	$(document).on('click', '.open-upload', function(event) {
		event.stopPropagation();
		event.preventDefault();
		$(".upload").show();
	...
	$(".upload-btn").click(function(event){
		...
		// 下面2行是客户端调用，自动返回的src，这里模拟，正事环境需要去掉isNotAjax來判断 ---------------------
		if(isNotAjax){
