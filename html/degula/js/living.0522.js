...
* @Last Modified time: 2017-05-25 16:41:56
*/
...
//2.0 请求数据
function getDate(){
	// $.post(domain+'v2/activity/dracula_final_data', {"HTTP_USER_TOKEN":token, "HTTP_USER_UID":pfid, "anchor_pfid":anchor_id },
	 	// function(data) {
...
function voteEvent(){
	$("#vote-box").on('click', '.vote-btn', function(event) {
		...
		// $.post(domain+'v2/activity/dracula_final_vote', {"anchor_pfid":votePfid },
		 	// function(response) {
				if(response.ret_code=="0"){
					var Remaining_count=response.data.ticket_count;
					var total_count=response.data.ticket_count;
					$(this).prev("p").find("span").html(total_count);
